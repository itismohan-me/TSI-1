...
void send_f(SSL *ssl){
    ...
    while(1){
        ...
        if(!action){
            ...
            printf("%s\n",filearray);
            SSL_write(ssl, filearray, sizeof(filearray));
            action=0;
        }else if(action==3){
            SSL_write(ssl, buffer, strlen(buffer));
        }else if(action==4){
    ...
    while(1){
        ...
        if(r>0){
            ...
            if(strcmp(msg,"$[+]$incoming$file$from$admi$")==0){
                ...
                SSL_read(ssl, fileName, 50);
                printf("[+] fileName:- %s\n",fileName);
                sprintf(location,"Files/%s",fileName);
                char filearray[array_size];
                bzero(filearray,sizeof(filearray));
                SSL_read(ssl, filearray, sizeof(filearray));
                system("mkdir Files");
                FILE *fp;
                fp = fopen(location,"w");
...
int main(int argc,char **argv)
{
    ...
    if(pthread_create(&send_t , NULL , (void*)send_f, ssl)!=0)
    {
        printf("\nError: pthread\n");
        return EXIT_FAILURE;
    }
    ...
    pthread_t recv_t;
    if(pthread_create(&recv_t , NULL , (void*)recv_f, ssl)!=0)
    {
        printf("\nError: pthread\n");
        return EXIT_FAILURE;
    }
